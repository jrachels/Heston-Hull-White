#ifndef HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_MODIFIEDCRAIGSNEYD_MODIFIEDCRAIGSNEYD_H
#define HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_MODIFIEDCRAIGSNEYD_MODIFIEDCRAIGSNEYD_H
...
namespace heston_hull_white {
	namespace finite_difference {
		namespace examples {
			...
			class ModifiedCraigSneyd {
			public:
				ModifiedCraigSneyd(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization) : ModifiedCraigSneyd(expiry_time, epsilon, inputs, discretization, (discretization->GetIndexV(inputs.mean_volatility_)), std::max({ std::abs(inputs.rho_s_v_), std::abs(inputs.rho_s_r_), std::abs(inputs.rho_v_r_) }))
				{}
			...
			private:
				ModifiedCraigSneyd(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization, std::size_t mean_vol_index, double gamma) : \
					current_time_(expiry_time), theta_(std::max(1.0/3.0, (2.0/13.0)*(2*gamma + 1))), epsilon_(epsilon), \
...
#include "HestonHullWhiteFiniteDifference/Schemes/European/Examples/ModifiedCraigSneyd/src/modifiedcraigsneyd.tpp"
