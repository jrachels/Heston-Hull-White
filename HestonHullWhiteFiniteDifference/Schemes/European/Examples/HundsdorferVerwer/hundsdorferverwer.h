#ifndef HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_HUNDSDORFERVERWER_HUNDSDORFERVERWER_H
#define HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_HUNDSDORFERVERWER_HUNDSDORFERVERWER_H
...
namespace heston_hull_white {
	namespace finite_difference {
		namespace examples {
			...
			class HundsdorferVerwer {
			public:
				HundsdorferVerwer(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization) : HundsdorferVerwer(expiry_time, epsilon, inputs, discretization, (discretization->GetIndexV(inputs.mean_volatility_)))
				{}
			...
			private:
				HundsdorferVerwer(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization, std::size_t mean_vol_index) : \
					current_time_(expiry_time), theta_((.5) + (1.0/6.0)*(std::sqrt(3))), epsilon_(epsilon), \
...
#include "HestonHullWhiteFiniteDifference/Schemes/European/Examples/HundsdorferVerwer/src/hundsdorferverwer.tpp"
