...
namespace heston_hull_white {
    namespace util {
        namespace internal {
            ...
            void funcgrad(double (*minfunc) (const fourier::HestonHullWhiteInputs&, const ap::real_1d_array&), const fourier::HestonHullWhiteInputs& hhw_inputs, const ap::real_1d_array& x, double& f, ap::real_1d_array& g)
            {	//This function stores the minimization function value at x in f and its gradient in g. Also see the remarks below.
                const double epsilon = 1e-8;		//"lengtht" of the numeric differential quotient to compute gradient
                const int xMIN = x.getlowbound();	//bounds of the components of x and g
                const int xMAX = x.gethighbound();
                const int gMIN = g.getlowbound();

                f = minfunc(hhw_inputs, x);				//minimization function call with params void pointer technique, see
                ap::real_1d_array xCopy = x;		//documentation, section 5.4.2

                for (int i = 0; i <= (xMAX - xMIN); i++) //Computes gradient.
                {
                    xCopy(xMIN + i) += epsilon;
                    g(gMIN + i) = (minfunc(hhw_inputs, xCopy) - f) / epsilon;
                    xCopy(xMIN + i) -= epsilon;
                }
            }


            void lbfgsbminimize(double (*minfunc) (const fourier::HestonHullWhiteInputs&, const ap::real_1d_array&), const fourier::HestonHullWhiteInputs& params, const int& n,
                const int& m,
                ap::real_1d_array& x,
                const double& epsg,
                const double& epsf,
