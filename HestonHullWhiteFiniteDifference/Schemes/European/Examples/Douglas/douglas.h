#ifndef HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_DOUGLAS_DOUGLAS_H
#define HESTONHULLWHITEFINITEDIFFERENCE_SCHEMES_EUROPEAN_EXAMPLES_DOUGLAS_DOUGLAS_H
...
namespace heston_hull_white {
	namespace finite_difference {
		namespace examples {
			...
			class Douglas {
			public:
				Douglas(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization) : Douglas(expiry_time, epsilon, inputs, discretization, (discretization->GetIndexV(inputs.mean_volatility_)))
				{}
			...
			private:
				Douglas(double expiry_time, double epsilon, HestonHullWhiteADIInputs inputs, std::shared_ptr<const ADIDiscretization> discretization, std::size_t mean_vol_index) : \
					current_time_(expiry_time), theta_(.7), epsilon_(epsilon), \
...
#include "HestonHullWhiteFiniteDifference/Schemes/European/Examples/Douglas/src/douglas.tpp"
