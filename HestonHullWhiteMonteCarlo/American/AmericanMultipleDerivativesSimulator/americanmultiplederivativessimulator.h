#ifndef HESTONHULLWHITEMONTECARLO_EUROPEAN_AMERICANMULTIPLEDERIVATIVESSIMULATOR_AMERICANMULTIPLEDERIVATIVESSIMULATOR_H
#define HESTONHULLWHITEMONTECARLO_EUROPEAN_AMERICANMULTIPLEDERIVATIVESSIMULATOR_AMERICANMULTIPLEDERIVATIVESSIMULATOR_H
...
namespace heston_hull_white {
	namespace monte_carlo {
		namespace american {
			...
			template<typename StateType, typename PathIndependentsSpecialization, typename PathDependentsSpecialization, typename StateTypeInitializationParameters, util::MatrixConcept MatrixType = util::Matrix>
			...
			class AmericanMultipleDerivativesSimulator {
			public:
				...
				AmericanMultipleDerivativesSimulator(PathIndependentsSpecialization& path_independents, PathDependentsSpecialization& path_dependents, double maturity_date, int num_subintervals, StateTypeInitializationParameters& inputs) : \
				...
				std::vector<MatrixType> RunFirstTrial(_Engine& rng_engine, int num_trials) {
					// constructs the matrices fills the first row of each matrix with the results of the first experiment
					StateType state = starting_state_;
					path_dependents_.ResetAndInitialize(); // unnecessary

					std::vector<double> initial_path_independent_prices = path_independents_.ComputePrices(state);
					std::vector<double> initial_path_dependent_prices = path_dependents_.ComputePrices(state);

					int total_number_derivatives = initial_path_independent_prices.size() + initial_path_dependent_prices.size();

					std::vector<MatrixType> result{ MatrixType(), total_number_derivatives };
